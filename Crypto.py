...
if not os.path.exists(file_path):
    ...
    try:
        lastest_date_change = int(
            datetime.strptime(lastest_date, "%Y-%m-%d %H:%M:%S.%f").timestamp()
        )
    except ValueError:
        try:
            lastest_date_change = int(
            ...
            try:
                lastest_date_change = int(
                    datetime.strptime(lastest_date, "%Y-%m-%d").timestamp()
                )
            except ValueError as exc:
                raise ValueError(
                    f"Unsupported date format: {lastest_date}") from exc
    ...
    Daily = Daily[['prices', 'Volume']]
    print(Daily)
    Daily.to_csv('crypto.csv')
